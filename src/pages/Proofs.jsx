...
const SECTION_META = {
  ...
  telegram: { label: '✈️ Telegram Proofs', description: 'Customers who bought via Telegram.', accent: '#3b82f6' },
  reddit:   { label: '🟠 Reddit Proofs',   description: 'Reddit DM proofs and vouches.', accent: '#f97316' },
}
  ...
  return (
    <>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 animate-fade-in-up">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-[var(--color-text)] mb-1" style={{ fontFamily: 'var(--font-heading)' }}>{meta.label}</h1>
        <p className="text-[var(--color-text-muted)] mb-6">{meta.description}</p>

        <a href={REDDIT_VOUCH_POST} target="_blank" rel="noreferrer"
          className="inline-flex items-center gap-2 mb-8 px-4 py-2.5 rounded-xl text-sm font-medium transition-all hover:translate-y-[-1px] bg-[var(--color-orange-dim)] border border-[var(--color-orange)]/20 text-[var(--color-orange)] hover:border-[var(--color-orange)]/40">
          📌 View our Reddit Vouch Post
